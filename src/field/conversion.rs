...
macro_rules! impl_from_unsigned {
    ($($t:ty),* $(,)?) => {
        $(
            impl<MOD: Monty<LIMBS>, const LIMBS: usize> From<$t> for RandomField<MOD, LIMBS> {
...
impl<MOD: Monty<LIMBS>, const LIMBS: usize> From<usize> for RandomField<MOD, LIMBS> {
...
impl<MOD: Monty<LIMBS>, const LIMBS: usize> From<bool> for RandomField<MOD, LIMBS> {
    fn from(value: bool) -> Self {
        if value { Self::ONE } else { Self::ZERO }
    }
}

impl<MOD: Monty<LIMBS>, const LIMBS: usize> From<Uint<LIMBS>> for RandomField<MOD, LIMBS> {
    ...
    for RandomField<MOD, LIMBS>
{
    fn from(value: Int<LIMBS>) -> Self {
        // if LIMBS >= N {
        let result = Self(Form::new(&value.abs()));
        ...
        /*} else {
            let wide_module = MOD::PARAMS.modulus().resize::<N>();
            let wide_value = value % NonZero::new(wide_module).unwrap();
            let wide_value = value.ab
        }*/
    }
}

impl<MOD: Monty<LIMBS>, const LIMBS: usize, T: Clone> From<&T> for RandomField<MOD, LIMBS>
where
    Self: From<T>,
{
    fn from(value: &T) -> Self {
...
mod tests {
    ...
    fn from_ref_generic() {
        let x: u64 = 77;
        let f1: F = RandomField::from(x);
        let f2: F = RandomField::from(&x);
        assert_eq!(f1, f2);
    }
}

impl<MOD: Monty<LIMBS>, const LIMBS: usize> MapIterable for RandomField<MOD, LIMBS> {
    fn map_iterable<'a, const M: usize, I: IntoIterator<Item = &'a Int<M>>>(
        iterable: I,
    ) -> Vec<Self> {
        iterable
            .into_iter()
            .map(|v| Self::from(v.resize()))
            .collect()
    }
}
