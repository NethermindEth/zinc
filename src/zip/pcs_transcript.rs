...
use crate::{
    ...
    traits::{BigInteger, ConfigReference, FromBytes, Integer, PrimitiveConversion, Words},
...
pub struct PcsTranscript<C: ConfigReference> {
...
impl<C: ConfigReference> PcsTranscript<C> {
    ...
    pub fn common_field_element(&mut self, fe: &RandomField<C>) {
    ...
    pub fn write_field_elements(&mut self, elems: &[RandomField<C>]) -> Result<(), Error> {
        for elem in elems {
    ...
    pub fn read_field_element(&mut self, config: C) -> Result<RandomField<C>, Error> {
        let mut bytes: Vec<u8> = vec![0; C::N * 8];

        self.stream
            .read_exact(&mut bytes)
            .map_err(|err| Error::Transcript(err.kind(), err.to_string()))?;

        let fe = RandomField::new_unchecked(config, C::B::from_bytes_be(&bytes).unwrap());
    ...
    pub fn write_field_element(&mut self, fe: &RandomField<C>) -> Result<(), Error> {
        self.common_field_element(fe);
    ...
    pub fn squeeze_challenge_idx(&mut self, config: C, cap: usize) -> usize {
...
macro_rules! test_read_write {
    ...
    ($write_fn:ident, $read_fn:ident, $original_value:expr, $assert_msg:expr) => {{
        ...
        let mut transcript = PcsTranscript::<ConfigRef<N>>::new();
        transcript
            .$write_fn(&$original_value)
            .expect(&format!("Failed to write {}", $assert_msg));
        let proof = transcript.into_proof();
        let mut transcript = PcsTranscript::<ConfigRef<N>>::from_proof(&proof);
