...
impl<const N: usize, const L: usize, const K: usize, const M: usize> RaaCode<N, L, K, M> {
    pub fn new<S: LinearCodeSpec, T: Transcript<L>>(
        ...
        let perm_1_seed = transcript.get_word();
        let perm_2_seed = transcript.get_word();
...
mod tests {
    ...
    //
    //     let repeated_output = repeat::<_, Int<I>>(&input, repetition_factor);
    //
