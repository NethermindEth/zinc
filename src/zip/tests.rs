...
fn test_zip_commitment() {
    ...
    let res = TestZip::commit::<ConfigRef<N>>(&param, &mle);
...
fn test_zip_opening() {
    ...
    let (data, _) = TestZip::commit::<ConfigRef<N>>(&param, &mle).unwrap();
...
fn test_failing_zip_evaluation() {
    ...
    let (data, comm) = TestZip::commit::<C>(&param, &mle).unwrap();

    let point = vec![0i64, 0i64, 0i64].map_to_field(config);
    let eval: F = 7i64.map_to_field(config);

    let mut transcript = PcsTranscript::new();
    let _ = TestZip::open::<C>(&param, &mle, &data, &point, config, &mut transcript);
...
fn test_zip_batch_evaluation() {
    ...
    let commitments: Vec<_> = TestZip::batch_commit::<C>(&param, &mles).unwrap();
